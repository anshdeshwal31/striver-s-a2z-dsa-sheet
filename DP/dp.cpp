...
// length of longest increasing subsequence using binary search - https://takeuforward.org/data-structure/longest-increasing-subsequence-binary-search-dp-43/
...
int solve(int i, int j, vector<int>& cuts) {
    ...
    for (int k = 0; k < cuts.size(); ++k) {
        if (cuts[k] <= i || cuts[k] >= j) continue; // skip invalid cuts

        cutMade = true;
        int cost = (j - i) + solve(i, cuts[k], cuts) + solve(cuts[k], j, cuts);
        mini = min(mini, cost);
    }
...
// Boolean evaluation - https://www.naukri.com/code360/problems/problem-name-boolean-evaluation_1214650
